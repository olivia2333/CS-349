...
public class LowerArm extends Sprite{
    ...
    public LowerArm(double start_x, double start_y, int width, int height, boolean left){
        super();
    ...
    private void initialize(double start_x, double start_y, double width, double height, boolean left){
        ...
        h = height;
        if (left) {
            image = new Image("left_lower_arm.png");
            pivot = new Point2D(x + w - 3, y+3);
            initial_pivot = new Point2D(x + w - 3, y+3);
        }
        else {
            image = new Image("right_lower_arm.png");
            pivot = new Point2D(x + 5, y+5);
            initial_pivot = new Point2D(x + 5, y+5);
        }
        this.left = left;
    }
